...
class picasaImageAdap extends ArrayAdapter<picasaImageContent> {
	Context context;
	List<picasaImageContent> MyList;
	...
	public picasaImageAdap(Context context, int LayoutResourceID,List<picasaImageContent> MyList,int count,GridView PhotoGrid,Cache_Options cop) {
	...
	public class DownloadImagesTask extends AsyncTask<List<picasaImageContent>,Integer,Void> {
		protected Void doInBackground(List<picasaImageContent>... params) {
			Integer cnt=0;
			while(cnt<params[0].size())
			{
				try {
					if(d[cnt]==null){
						...
						d[cnt]=cp.image_load(strngs.SITE_PICASA, strngs.TYPE_IMAGE_THUMB, id, URL, Cache_Options.ALLOW_CACHE);
						publishProgress(cnt);
					}
